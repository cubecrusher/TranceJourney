...
public class Obstacle2 {
    ...
    public Obstacle2(GameScreen gameScreen){
    ...
    public void update(){
        if (isOut) {
            this.pos = getRandom();
            if (pos == 0) Obstacle2.this.x = 0;
            if (pos == 1) Obstacle2.this.x = TrJr.INSTANCE.getScrW() / 5f;
            if (pos == 2) Obstacle2.this.x = 2 * TrJr.INSTANCE.getScrW() / 5f;
            if (pos == 3) Obstacle2.this.x = 3 * TrJr.INSTANCE.getScrW() / 5f;
            if (pos == 4) Obstacle2.this.x = 4 * TrJr.INSTANCE.getScrW() / 5f;
            isOut = false;
        }
        if (y<-200) {
            isOut = true;
